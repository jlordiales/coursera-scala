...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers") {
    assert(sumRegularRecursive(List(1,2,0)) === 3)
  }
  
  test("sum of an empty list is zero") {
    assert(sumRegularRecursive(List()) === 0)
  }
  
  test("sum of a list with one element is that element") {
    assert(sumRegularRecursive(List(4)) === 4)
  }
  
  test("sum tail recursive of a few numbers") {
    assert(sumTailRecursive(List(1,2,0)) === 3)
  }
  
  test("sum tail recursive of an empty list is zero") {
    assert(sumTailRecursive(List()) === 0)
  }
  
  test("sum tail recursive of a list with one element is that element") {
    assert(sumTailRecursive(List(4)) === 4)
  }
  ...
  test("max of a list with just one element") {
    assert(max(List(2)) === 2)
  }
  
  test("max of list with negatives numbers") {
    assert(max(List(-1,-3,-5,-6)) === -1)
  }
  
  test("max of list with repeated max value") {
    assert(max(List(1,7,7,6,5)) === 7)
  }
}
